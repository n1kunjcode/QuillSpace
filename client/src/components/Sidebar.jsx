...
import { Folder, FileText, Plus, Search, ChevronRight, ChevronDown, Edit2, Trash2, PanelLeftClose, PanelLeft, X, LogOut } from "lucide-react";
...
const css = `
  ...
  /* ── FIX: hide the desktop collapse button when on mobile ── */
  .sb-collapse-btn {
    display: flex;
  }
  ...
  .sb-search-clear {
    position: absolute; right: 8px; background: none; border: none; color: #444;
    cursor: pointer; display: flex; align-items: center; padding: 2px; transition: color 0.2s;
  }
  ...
  .sb-row-left { display: flex; align-items: center; gap: 7px; flex: 1; min-width: 0; overflow: hidden; }
  .sb-row-name { font-size: 13px; font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; flex: 1; }
  ...
  .sb-new-note-btn {
    ...
    padding: 9px 12px; background: none; border: none; border-radius: 9px;
    color: #555; font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 500;
  ...
  .sb-new-note-icon {
    width: 20px; height: 20px; background: rgba(74,222,128,0.12); border-radius: 5px;
  ...
  .sb-avatar {
    ...
    display: flex; align-items: center; justify-content: center;
    color: #f59e0b; flex-shrink: 0; font-size: 12px; font-weight: 600;
  ...
  .sb-collapsed {
    height: 100%; border-right: 1px solid rgba(255,255,255,0.07);
...
export default function Sidebar({ onNavigate }) {
  ...
  const filteredFolders = searchQuery
    ? sortedFolders.filter(f => f.name.toLowerCase().includes(searchQuery.toLowerCase()))
    : sortedFolders;
  const filteredNotes = searchQuery
    ? sortedNotes.filter(n =>
        n.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
        (n.folderId && folders.find(f => f._id === n.folderId)?.name.toLowerCase().includes(searchQuery.toLowerCase()))
      )
    : sortedNotes;
  const getFolderName = (note) => note.folderId ? folders.find(f => f._id === note.folderId)?.name : null;

  const initials = userMeta?.name
    ? userMeta.name.split(" ").map(w => w[0]).join("").slice(0, 2).toUpperCase()
    : "?";

  // ── Collapsed view (desktop only — mobile never shows this) ────
  if (collapsed) {
    return (
      <>
        <style>{css}</style>
        ...
        <Modal isOpen={inputModal.isOpen} onClose={() => setInputModal({ ...inputModal, isOpen: false })} title="New Folder"
          actions={<>
            <button onClick={() => setInputModal({ ...inputModal, isOpen: false })} style={cancelBtnStyle}>Cancel</button>
            <button onClick={handleInputSubmit} style={confirmBtnStyle}>Create</button>
          </>}>
          <input autoFocus className="modal-input" placeholder="Folder name…" value={inputModal.value}
    ...
    <>
      ...
      <motion.div className="sb-root" initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.4 }}>
        ...
        <div className="sb-header">
          ...
          {/* ── FIX: collapse button has sb-collapse-btn class, hidden via CSS on mobile ── */}
          <button className="sb-icon-btn sb-collapse-btn" onClick={() => setCollapsed(true)} title="Collapse">
        ...
        <div className="sb-search-wrap">
          <div className="sb-search-inner">
            ...
            <input ref={searchInputRef} className="sb-search-input" type="text" placeholder="Search…"
              ...
              <motion.div key="search" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                {filteredFolders.length > 0 && <>
                  ...
                  {filteredFolders.map(f => (
                    <SbRow key={f._id} label={f.name} icon={<Folder size={14} />}
                      active={activeFolder?._id === f._id && !activeNote}
                      onClick={() => handleFolderClick(f)}
                      onRename={() => openInputModal("renameFolder", f)}
                      onDelete={() => confirmDelete("folder", f)}
                ...
                {filteredNotes.length > 0 && <>
                  ...
                  {filteredNotes.map(n => {
                    ...
                    return <SbRow key={n._id} label={n.title || "Untitled"} subtitle={fn}
                      icon={<FileText size={14} />} active={activeNote?._id === n._id}
                      onClick={() => handleNoteClick(n, fn ? folders.find(f => f._id === n.folderId) : null)}
                      onRename={() => openInputModal("renameNote", n)}
                      onDelete={() => confirmDelete("note", n)} />;
                  })}
                </>}
              ...
              <motion.div key="normal" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                {sortedFolders.length > 0 && <>
                  ...
                  {sortedFolders.map(f => {
                    ...
                    return (
                      <div key={f._id}>
                        <SbRow label={f.name} icon={<Folder size={14} />}
                          active={activeFolder?._id === f._id && !activeNote}
                          expanded={expanded}
                          onToggle={e => { e.stopPropagation(); toggleFolder(f._id); }}
                          onClick={() => handleFolderClick(f)}
                          onRename={() => openInputModal("renameFolder", f)}
                          onDelete={() => confirmDelete("folder", f)}
                          badge={fNotes.length || null} />
                        <AnimatePresence>
                          {expanded && (
                            <motion.div className="sb-children"
        ...
        <div className="sb-user">
          <div className="sb-avatar">
            {userMeta?.picture
              ? <img src={userMeta.picture} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
              : initials
            }
          </div>
      ...
      <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)}
        ...
        actions={<>
          ...
          <button onClick={deleteItem} style={deleteBtnStyle}>Delete</button>
        </>}>
      ...
      <Modal isOpen={inputModal.isOpen} onClose={() => setInputModal({ ...inputModal, isOpen: false })}
        ...
        actions={<>
          <button onClick={() => setInputModal({ ...inputModal, isOpen: false })} style={cancelBtnStyle}>Cancel</button>
          <button onClick={handleInputSubmit} style={confirmBtnStyle}>{inputModal.type === "createFolder" ? "Create" : "Save"}</button>
        </>}>
